...
/// clip.sol -- Blc auction module 2.0
...
interface VatLike {
    ...
    function ilks(bytes32) external returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256);
...
contract Clipper {
    ...
    address     public vow;      // Recipient of blc raised in auctions
    ...
    // 
    // Auctions will not collect more BLC than their assigned BLC target,`tab`;
    // thus, if `amt` would cost more BLC than `tab` at the current price, the
    // amount of collateral purchased will instead be just enough to collect `tab` BLC.
    //
    // To avoid partial purchases resulting in very small leftover auctions that will
    // never be cleared, any partial purchase must leave at least `Clipper.chost`
    // remaining BLC target. `chost` is an asynchronously updated value equal to
    ...
    function take(
        ...
        uint256 max,          // Maximum acceptable price (BLC / collateral) [ray]
...

        {
            ...
            // BLC needed to buy a slice of this sale
            owe = mul(slice, price);

            // Don't collect more than tab of BLC
            if (owe > tab) {
            ...
            // Removes Blc out for liquidation from accumulator
